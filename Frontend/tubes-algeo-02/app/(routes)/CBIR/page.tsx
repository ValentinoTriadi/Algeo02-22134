...
const CBIR: React.FC = () => {
  ...
  // Fungsi mengganti nama dan tampilan file
  const inputImageHandler = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      const file = e.target.files[0];
      if (file) {
        setImageName(file.name.substring(0, 40));
        const imgUrl = URL.createObjectURL(file);
        setImageUrl(imgUrl); // Hiraukan ini kalau merah
      }
    }
  }
    ...
    <section className='w-full flex flex-col items-center py-8 justify-center'>
      ...
      <div className="w-[400px] md:w-[750px] mt-[75px] border-t border-1 border-black py-4 flex flex-col items-center gap-y-4">
        ...
        <Button 
          ...
          className="border-2 rounded-full border-white hover:bg-white hover:text-black hover:border-2 hover:border-black">
